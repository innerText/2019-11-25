...
function render(num=0){
    ...
    $.each(d,(i,item)=>{
        ...
        let $folder = $(`<div id="box" did = ${id}> <i class="iconfont icon-weibiaoti-_huabanfuben"></i><input type="text" value="${title}" class="editor"/><span>${title}</span></div>
        `);

        let $img = $folder.find('i');
        $folder.find('input').css("display","none");
